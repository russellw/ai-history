...
# Ingest + superset safeguard (Claude, Grok, Qwen)

These three converters share a pipeline. You drop the freshly downloaded file(s) into the provider directory and run the script, which:

1. Determines the download's date (Claude: from `manifest-*.json`; Grok: from the zip's internal file timestamps; Qwen: from the `chat-export-<epoch-ms>.json` filename — each falling back to file mtime).
...
Re-running with no new download present just re-converts the latest dated subdirectory. ChatGPT does **not** use this pipeline — it reads `ChatGPT/conversations-*.json` in place.
